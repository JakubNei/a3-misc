/*
	...
	DESCRIPTION:
	Found amazing addon that allows you to put backpack intro ventral position.
	...
	Fixed bug where it did not work once you respawned with your old body having backpack in ventral position.
	Fixed bug? where the addActions on your old body would stay after you respawned.
...
FSFSV_Player_Init = {
	...
	local _FSFSV_SacADosGwh = player getVariable "FSFSV_BACKPACK";
	if(!isNil{_FSFSV_SacADosGwh} && {!isNull _FSFSV_SacADosGwh}) then {
		detach _FSFSV_SacADosGwh;
	};

	if(!isNil{FSFSV_OldPlayer}) then {
		{
			FSFSV_OldPlayer removeAction _x;	
		} forEach FSFSV_AddActionsIds;
	};

	player forceWalk false;
	player setVariable ["FSFSV_BACKPACK",objNull,true];

	FSFSV_AddActionsIds = [];
	FSFSV_AddActionsIds pushBack ( 
...
player addEventHandler ["Respawn",{[] spawn FSFSV_Player_Init;}];
[] spawn FSFSV_Player_Init;
