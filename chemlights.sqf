...
GVAR(menu_main)={
	call aero_gm_actions_remove;
	{
		...
		["<t color='#B40404'>Attach Red Chemlight ...</t>", { _THIS(3) call GVAR(shoulder_menu); }, ["B40404","Chemlight_red"],5090],
		["<t color='#30fd07'>Attach Green Chemlight ...</t>", { _THIS(3) call GVAR(shoulder_menu); }, ["30fd07","Chemlight_green"],5080],
		["<t color='#68ccf6'>Attach Blue Chemlight ...</t>", { _THIS(3) call GVAR(shoulder_menu); }, ["68ccf6","Chemlight_blue"],5070],
		["<t color='#fcf018'>Attach Yellow Chemlight ...</t>", { _THIS(3) call GVAR(shoulder_menu); }, ["fcf018","Chemlight_yellow"],5060]
	];	
	if(count GVAR(attached)>0) then {
		(player addAction ["<t color='#cccccc'>Detach chemlights</t>", { _THIS(3) call GVAR(detach_chemlights); },[],3000]) call aero_gm_actions_addId;		
	};
...
GVAR(shoulder_menu)={
	call aero_gm_actions_remove;	
	{
		...
		[format["<t color='#%1'>Attach To Left Shoulder</t>",_THIS(0)], { _THIS(3) call GVAR(attach_chemlight); }, [_THIS(1),"LeftShoulder",[-0.02,-0.05,0.04]],5090],
		[format["<t color='#%1'>Attach To Right Shoulder</t>",_THIS(0)], { _THIS(3) call GVAR(attach_chemlight); }, [_THIS(1),"RightShoulder",[0.02,-0.05,0.04]],5080],
		[format["<t color='#%1'>Attach To Left Hand</t>",_THIS(0)], { _THIS(3) call GVAR(attach_chemlight); }, [_THIS(1),"LeftHand",[0,0,0]],5070],
		[format["<t color='#%1'>Attach To Right Hand</t>",_THIS(0)], { _THIS(3) call GVAR(attach_chemlight); }, [_THIS(1),"RightHand",[0,0,0]],5060]
	];
